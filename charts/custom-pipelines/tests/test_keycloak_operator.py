...
global:
  gitProviders:
    ...
    review = f"{vcs}-go-kc-other-app-review"
    build = f"{vcs}-go-kc-other-app-build-edp"
    ...
    assert "kaniko-build" in r[11]["name"]
    assert "e2e" in r[12]["name"]
    ...
    assert "save-cache" in b[10]["name"]
