...
global:
  gitProvider: gerrit
    ...
    review = f"{vcs}-go-so-other-app-review"
    build = f"{vcs}-go-so-other-app-build-edp"
    ...
    assert "commit-validate" in r[4]["name"]
    ...
    assert "kaniko-build" in r[12]["name"]
    assert "e2e" in r[13]["name"]
    assert "save-cache" in r[14]["name"]
