...
| pipelines.autotestsServiceAccount.enabled | bool | `false` | Create the `krci-autotests-tmp` ServiceAccount. Enable only on environments the test automation framework runs against; it has no consumer anywhere else. Remove together with the constant in the framework, once that resolves the account the way the trigger path does. |
