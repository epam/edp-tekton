...
# массивы для параметров и тасок
PARAM_BLOCKS=()
TASK_BLOCKS=()

# добавить параметр/таску (блок должен быть уже с нужными отступами)
add_param_block() { PARAM_BLOCKS+=("$1"); }
add_task_block()  { TASK_BLOCKS+=("$1"); }

# склейка массивов в одну строку для heredoc
join_params() { printf "%s\n" "${PARAM_BLOCKS[@]}"; }
join_tasks()  { printf "%s\n" "${TASK_BLOCKS[@]}"; }

# ------------------------- общие параметр-блоки (по одному) --------------------
...
# ----------------- провайдер/тип: ОТДЕЛЬНЫЕ параметры (по одному) --------------
  ...
  if [[ "$TYPE" == "build" ]]; then
    ...
    case "$VCS" in
      ...
      gerrit)
        ...
        task_git_tag
        task_update_cbis
        VOTE_INCLUDE='' # нет отдельного include
        ;;
    esac
  else
