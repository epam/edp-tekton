  ...
  dockerRegistry:
    ...
    for buildtool in ['go']:
        for framework in ['beego', 'gin', 'operator-sdk']:
            ...
            assert "golang" == btd[7]["taskRef"]["name"]
            ...
            assert "golang" == btedp[7]["taskRef"]["name"]
            assert "create-ecr-repository" in btd[8]["name"]
  ...
  dockerRegistry:
    ...
    for buildtool in ['go']:
        for framework in ['beego', 'gin', 'operator-sdk']:
            ...
            assert "golang" == btd[3]["taskRef"]["name"]
            assert "sonar" in btd[4]["name"]
            assert "sonarqube-scanner" == btd[4]["taskRef"]["name"]
            assert "build" in btd[5]["name"]
            assert "golang" == btd[5]["taskRef"]["name"]
            assert "create-ecr-repository" in btd[6]["name"]
            assert "kaniko-build" in btd[7]["name"]
            assert "sast" in btd[8]["name"]
            ...
            assert "golang" == btedp[3]["taskRef"]["name"]
            assert "sonar" in btedp[4]["name"]
            assert "sonarqube-scanner" == btedp[4]["taskRef"]["name"]
            assert "build" in btedp[5]["name"]
            assert "golang" == btedp[5]["taskRef"]["name"]
            assert "create-ecr-repository" in btd[6]["name"]
            assert "kaniko-build" in btedp[7]["name"]
            assert "sast" in btedp[8]["name"]
