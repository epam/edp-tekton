        ...
        for framework in ['java8', 'java11', 'java17']:
            for cbtype in ['app', 'lib']:
                ...
                if cbtype == "app":
                    ...
                    assert "fetch-target-branch" in rt[8]["name"]
                    assert "sonar-prepare-files" in rt[9]["name"]
                    assert f"sonar-prepare-files-{buildtool}" == rt[9]["taskRef"]["name"]
                    assert "sonar" in rt[10]["name"]
                    assert "build" in rt[11]["name"]
                    assert "save-cache" in rt[12]["name"]
                    assert "dockerfile-lint" in rt[13]["name"]
                    assert "dockerbuild-verify" in rt[14]["name"]
                    assert "helm-lint" in rt[15]["name"]
                if cbtype == "lib":
                    assert "fetch-repository" in rt[0]["name"]
                    assert "gerrit-notify" in rt[1]["name"]
                    assert "init-values" in rt[2]["name"]
                ...
                if cbtype == "app":
                    ...
                    assert "sonar-cleanup" in btd[7]["name"]
                    assert "security" in btd[8]["name"]
                    assert "compile" in btd[9]["name"]
                    assert buildtool == btd[9]["taskRef"]["name"]
                    assert "test" in btd[10]["name"]
                    assert buildtool == btd[10]["taskRef"]["name"]
                    assert "sonar" in btd[11]["name"]
                    assert buildtool == btd[11]["taskRef"]["name"]
                    assert "build" in btd[12]["name"]
                    assert buildtool == btd[12]["taskRef"]["name"]
                    assert "get-nexus-repository-url" in btd[13]["name"]
                    assert "push" in btd[14]["name"]
                    assert buildtool == btd[14]["taskRef"]["name"]
                    assert "kaniko-build" in btd[15]["name"]
                    assert "save-cache" in btd[16]["name"]
                    assert "git-tag" in btd[17]["name"]
                    assert "update-cbis" in btd[18]["name"]
                else:
                ...
                if cbtype == "app":
                    ...
                    assert "sonar-cleanup" in btedp[7]["name"]
                    assert "security" in btedp[8]["name"]
                    assert "compile" in btedp[9]["name"]
                    assert buildtool == btedp[9]["taskRef"]["name"]
                    assert "test" in btedp[10]["name"]
                    assert buildtool == btedp[10]["taskRef"]["name"]
                    assert "sonar" in btedp[11]["name"]
                    assert buildtool == btedp[11]["taskRef"]["name"]
                    assert "build" in btedp[12]["name"]
                    assert buildtool == btedp[12]["taskRef"]["name"]
                    assert "get-nexus-repository-url" in btedp[13]["name"]
                    assert "push" in btedp[14]["name"]
                    assert buildtool == btedp[14]["taskRef"]["name"]
                    assert "kaniko-build" in btedp[15]["name"]
                    assert "save-cache" in btedp[16]["name"]
                    assert "git-tag" in btedp[17]["name"]
                    assert "update-cbis" in btedp[18]["name"]
                else:
