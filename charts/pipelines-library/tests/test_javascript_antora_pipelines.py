...
def test_javascript_antora_pipelines_gerrit():
    config = """
global:
  gitProvider: gerrit
  dockerRegistry:
    ...
    for buildtool in ['npm']:
        for framework in ['antora']:
            for cbtype in ['app']:
                ...
                assert "build" in rt[2]["name"]
                assert "dockerfile-lint" in rt[3]["name"]
                assert "dockerbuild-verify" in rt[4]["name"]
                assert "helm-lint" in rt[5]["name"]
                ...
                assert "create-ecr-repository" in btedp[6]["name"]
...
def test_javascript_antora_pipelines_github():
    config = """
global:
  gitProvider: github
  dockerRegistry:
...
def test_javascript_antora_pipelines_gitlab():
    config = """
global:
  gitProvider: gitlab
  dockerRegistry:
    ...
    for buildtool in ['npm']:
        for framework in ['antora']:
            for cbtype in ['app']:
                ...
                assert "build" in btd[3]["name"]
                assert buildtool == btd[3]["taskRef"]["name"]
                assert "kaniko-build" in btd[4]["name"]
