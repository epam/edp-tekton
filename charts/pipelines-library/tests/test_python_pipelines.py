...
# FastApi
def test_python_fastapi_pipelines_gerrit():
    config = """
global:
  gitProvider: gerrit
dockerRegistry:
  type: "ecr"
    ...
    assert f"get-version-{buildtool}-default" == bd[3]["taskRef"]["name"]
    ...
    assert "lint" in bedp[7]["name"]
    assert buildtool == bedp[7]["taskRef"]["name"]
    assert "compile" in bedp[8]["name"]
...
# Flask
def test_python_flask_pipelines_gerrit():
    config = """
global:
  gitProvider: gerrit
dockerRegistry:
...
def test_python_flask_pipelines_gitlab():
    config = """
global:
  gitProvider: gitlab
dockerRegistry:
...
def test_python_fastapi_pipelines_gitlab():
    config = """
global:
  gitProvider: gitlab
dockerRegistry:
