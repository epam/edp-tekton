        ...
        for framework in ['python-3.8']:
            for cbtype in ['app', 'lib']:
                ...
                if cbtype == "lib":
                    ...
                    assert "python" in rt[3]["name"]
                    assert "fetch-target-branch" in rt[4]["name"]
                    assert "sonar-prepare-files" in rt[5]["name"]
                    assert "sonar-prepare-files-general" == rt[5]["taskRef"]["name"]
                ...
                assert "python" in btd[7]["name"]
                assert "edp-python" == btd[7]["taskRef"]["name"]
                ...
                assert "python" in btedp[7]["name"]
                assert "edp-python" == btedp[7]["taskRef"]["name"]
    ...
    for buildtool in ['python']:
        for framework in ['python-3.8']:
            for cbtype in ['app', 'lib']:
                ...
                assert "python" in btd[5]["name"]
                assert "edp-python" == btd[5]["taskRef"]["name"]
                ...
                assert "python" in btedp[5]["name"]
                assert "edp-python" == btedp[5]["taskRef"]["name"]
...
global:
  gitProvider: gerrit
    ...
    assert "python" in r[4]["name"]
    assert "fetch-target-branch" in r[5]["name"]
    assert "sonar-prepare-files" in r[6]["name"]
    assert "sonar-prepare-files-general" == r[6]["taskRef"]["name"]
    ...
    assert "python" in bd[7]["name"]
    assert "edp-fastapi" == bd[7]["taskRef"]["name"]
    ...
    assert "python" in bedp[7]["name"]
    assert "edp-fastapi" == bedp[7]["taskRef"]["name"]
...
global:
  gitProvider: gitlab
    ...
    assert "update-build-number" in bd[3]["name"]
    assert "security" in bd[4]["name"]
    assert "python" in bd[5]["name"]
    assert "edp-fastapi" == bd[5]["taskRef"]["name"]
    ...
    assert "update-cbis" in bd[11]["name"]
    assert "push-to-jira" in ht["pipeline"][build_default]["spec"]["finally"][0]["name"]
    ...
    assert "update-build-number" in bedp[3]["taskRef"]["name"]
    assert f"update-build-number-{buildtool}" == bedp[3]["taskRef"]["name"]
    assert "security" in bedp[4]["name"]
    assert "python" in bedp[5]["name"]
    assert "edp-fastapi" == bedp[5]["taskRef"]["name"]
    assert "sonar" in bedp[6]["name"]
    assert "sonarqube-scanner" == bedp[6]["taskRef"]["name"]
